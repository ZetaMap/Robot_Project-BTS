...
bool NiryoInfo::actualiserAxes()
{
    ...
    for(int i = 0 ; i < 6 ; i++){
        this->axesAngles[i] = reponse.value(i);
    }
    return true;
...
bool NiryoInfo::actualiserPosition()
{
    ...
    for(int i = 0 ; i < 3 ; i++){
        this->positionsXYZ[i] = reponse.value(i);
    }
    return true;
}
...
bool NiryoInfo::actualiserOrientation()
{
    ...
    for(int i = 0 ; i < 3 ; i++){
        this->rollPitchYaw[i] = reponse.value(i);
    }
    return true;
}
...
bool NiryoInfo::actualiserInfoDiverse()
{
    QList<quint16> reponse;
    bool ok = false;

    reponse = controlleur->recupererInfoDiverse(ok);
    if(!ok){
        return false;
    } else {
        this->motorsConnection = NiryoController::quint16ToBool(reponse.value(0));
        this->calibrationRequise = NiryoController::quint16ToBool(reponse.value(1));
        this->calibrationEnCour = NiryoController::quint16ToBool(reponse.value(2));
        this->temperature = reponse.value(3);
...
void NiryoInfo::debugDump()
{
    ...
    qDebug() << "calibration requise: " << calibrationRequise << "\ncalibration en cour: " << calibrationEnCour;
}

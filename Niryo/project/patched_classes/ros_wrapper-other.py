...
class NiryoRosWrapper(AbstractNiryoRosWrapper):
    ...
    def get_saved_pose_list(self, with_desc=False):
        ...
        :type with_desc: bool
        :return: list of positions name, and positions descriptions if 'with_desc is True
    ...
    def set_conveyor(self):
        ...
        return result.status, result.message, self.__conveyor_id_to_conveyor_number(result.id)
    ...
    def get_target_pose_from_cam(self, workspace_name, height_offset, shape, color):
        ...
        """
        status, object_found, rel_pose, obj_shape, obj_color = self.detect_object(workspace_name, shape, color)
        if not object_found:
            return False, None, "", ""
    ...
    def detect_object(self, workspace_name, shape, color):
        ...
        :param color: color of the target
        :type color: ObjectColor
        :return: status, object_found, object_pose, object_shape, object_color
        :rtype: (int, bool, RobotState, str, str)
        """
        from niryo_robot_vision.srv import ObjDetection

        status, ratio = self.get_workspace_ratio(workspace_name)
        response = self._call_service("/niryo_robot_vision/obj_detection_rel", ObjDetection, shape, color, ratio, False)

        if status == CommandStatus.MARKERS_NOT_FOUND:
            rospy.logwarn_throttle(1, 'ROS Wrapper - Markers Not Found')
        elif status == CommandStatus.VIDEO_STREAM_NOT_RUNNING:
            rospy.logwarn_throttle(1, 'Video Stream not running')

        object_detected = status == CommandStatus.SUCCESS
        return status, object_detected, response.obj_pose, response.obj_type, response.obj_color
    ...
    def get_workspace_list(self, with_desc=False):
        ...
        :return: list of workspaces name, and workspaces descriptions if 'with_desc' is True
        :rtype: list[str], list[str] if with_desc is True
        """

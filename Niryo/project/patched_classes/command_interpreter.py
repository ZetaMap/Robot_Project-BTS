...
class CommandInterpreter:
    ...
    def __set_conveyor(self):
        status, message, conveyor_id = self.__niryo_robot.set_conveyor()
    ...
    def __detect_object(self, *param_list):
        status, object_found, rel_pose, shape, color = self.__niryo_robot.detect_object(*param_list)
        if not object_found:
    ...
    def __get_workspace_ratio(self, workspace_name):
        status, ratio = self.__niryo_robot.get_workspace_ratio(workspace_name)
        return self.__send_answer(ratio)

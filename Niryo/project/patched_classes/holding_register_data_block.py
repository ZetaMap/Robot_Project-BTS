...
HR_CLEAR_COLLISON_DETECTED = 103
HR_STOP_COMMAND = 110
...
# Error codes
ERROR_NO_RESULT_YEET = 0
ERROR_COMMAND_SUCCESS = 1
ERROR_COMMAND_REJECTED = 2
ERROR_COMMAND_ABORTED = 3
ERROR_COMMAND_CANCELLED = 4
ERROR_COMMAND_UNKNOWN_STATUS = 5
ERROR_COMMAND_TIMEOUT = 6
ERROR_INTERNAL_FAILURE = 7
ERROR_COMMAND_IS_RUNNING = 8
ERROR_COLLISION_DETECTED = 9
...
class HoldingRegisterDataBlock(DataBlock):
    ...
    def __set_command_in_progress(self):
        ...
        :rtype:
        """
        self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_NO_RESULT_YEET)
    ...
    def __set_command_done(self, status_result):
        ...
        :rtype:
        """
        self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, self.__get_status_code(status_result))
        ...
        def wrapper():
            ...
            try:
                try: response = func(*args)
                    ...
                    logging.getLogger("rospy.internal").error("ModbusAsyncAction - "+type(e).__name__+": "+str(e))
                    if isinstance(e, NiryoRosWrapperException):
                        if e.status is not None:
                            self.update_status(e.status)
                        elif "clear_collision_detected" in str(e).strip(): 
                            self.__set_command_done(ERROR_COLLISION_DETECTED)
                        else: self.update_status(None)
                else: self.update_status(response)
            except: self.__set_command_done(ERROR_INTERNAL_FAILURE)
            self.is_action_client_running = False
    ...
    def check_move_running(self):
        if self.is_action_client_running or self.execution_thread.is_alive():
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_IS_RUNNING)
            return True
        return False
    ...
    def setValues(self, address, values):
        try: 
            ...
            logging.getLogger("rospy.internal").error(type(e).__name__+": "+str(e))
            self.is_action_client_running = False
    ...
    def process_command(self, address, values):
        ...
        elif address == HR_LEARNING_MODE:  # done
            self.activate_learning_mode(values[0])
        elif address == HR_MOVE_JOINTS_COMMAND:  # done
            if not self.check_move_running(): self.move_joints_command()
        elif address == HR_MOVE_POSE_COMMAND:  # done
            if not self.check_move_running(): self.move_pose_command()
        elif address == HR_MOVE_LINEAR_POSE_COMMAND:
            if not self.check_move_running(): self.move_linear_pose_command()
        elif address == HR_CLEAR_COLLISON_DETECTED:
            self.python_ros_wrapper.clear_collision_detected()
    ...
    def request_new_calibration(self):
        ...
        self.__set_command_done(response)
    ...
    def stop_current_command(self):
        if self.is_action_client_running:
            self.python_ros_wrapper.stop_move()
            self.__set_command_done(ERROR_COMMAND_CANCELLED)
    ...
    def update_tool(self):
        ...
        self.setValuesOffset(HR_TOOL_ID, self.python_ros_wrapper.get_current_tool_id())
        self.__set_command_done(response)
    ...
    def move_joints_command(self):
        ...
        self.move_joints([handle_negative_hr(j) / MULT for j in joints_raw_values])

    def move_pose_command(self):
        self.__set_command_in_progress()
        pose_raw_values = self.getValuesOffset(HR_POSITION_X, 6)
        self.move_pose([handle_negative_hr(p) / MULT for p in pose_raw_values])

    def move_linear_pose_command(self):
        self.__set_command_in_progress()
        pose_raw_values = self.getValuesOffset(HR_POSITION_X, 6)
        self.move_linear_pose([handle_negative_hr(p) / MULT for p in pose_raw_values])
    ...
    def pull_air_vacuum_pump(self):
        self.__set_command_in_progress()
        response = self.python_ros_wrapper.grasp_with_tool()
        self.update_status(response)
    ...
    def set_motor_speed(self, speed):
        self.__set_command_in_progress()
        response = self.python_ros_wrapper.set_arm_max_velocity(speed)
        self.update_status(response)

    def move_pose(self, pose):
        self.__run_action_async(self.python_ros_wrapper.move_pose, *pose)
        
    def move_linear_pose(self, pose):
        self.__run_action_async(self.python_ros_wrapper.move_linear_pose, *pose)
        
    def move_joints(self, joints):
        self.__run_action_async(self.python_ros_wrapper.move_joints, *joints)
    ...
    def update_status(self, status):
        ...
        if status == CommandStatus.NO_CONNECTION or status == None:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_INTERNAL_FAILURE)
        elif status == CommandStatus.GOAL_TIMEOUT:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_TIMEOUT)
        elif status == CommandStatus.REJECTED:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_REJECTED)
        elif status == CommandStatus.ABORTED:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_ABORTED)
        elif status == CommandStatus.PREEMPTED:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_CANCELLED)
        elif status != CommandStatus.SUCCESS:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_UNKNOWN_STATUS)
        else:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_RESULT, ERROR_COMMAND_SUCCESS)
    ...
    def ping_and_set_conveyor(self):
        ...
        if response[0] == CommandStatus.SUCCESS:
            self.setValuesOffset(HR_LAST_ROBOT_CMD_DATA_RESULT, int(response[2]))
            self.setValuesOffset(HR_CONTROL_CONVEYOR_ID, int(response[2]))
    ...
    def enable_tcp(self, enable):
        ...
        return self.__set_command_done(response)
    ...
    def set_tcp(self, tcp_rpy_raw_values):
        ...
        response = self.python_ros_wrapper.set_tcp(*tcp)
        return self.__set_command_done(response)
    ...
    def get_target_pose_from_rel(self, height_offset=None, x_rel=None, y_rel=None, yaw_rel=None, sub_called=False):
        ...
        if height_offset is None: height_offset = self.height_offset
        if x_rel is None: x_rel = self.x_rel
        if y_rel is None: y_rel = self.y_rel
        if yaw_rel is None: yaw_rel = self.yaw_rel

        if any(elem is None for elem in [x_rel, y_rel, yaw_rel]):
            if not sub_called:
                self.setValuesOffset(HR_VISION_TARGET_POSE, [0] * 6)
                rospy.logwarn('Modbus Server - Unable to get target pose : x_rel, y_rel or yaw_rel requested is None')
                return
            else:
                return CommandStatus.INVALID_PARAMETERS, self.null_robot_state

        if not sub_called: self.__set_command_in_progress()
        ...
        if response.status == CommandStatus.SUCCESS:
            ...
            if not sub_called:
                self.setValuesOffset(HR_VISION_TARGET_POSE, [0,0,0,0,0,0])
                self.__set_command_done(response)
            else:
    ...
    def move_to_object(self):
        ...
        response_status, obj_found, obj_pose, obj_type_str, obj_color_str = self.get_target_pose_from_cam(sub_called=True)
    ...
    def detect_object(self, sub_called=False):
        ...
        if any(elem is None for elem in [self.workspace_name, self.vision_shape, self.vision_color]):
            if not sub_called:
                ...
                rospy.logwarn('Modbus Server - Unable to detect object : Workspace name, Shape or Color requested is None')
                return
            else:
        ...
        if response is None: return CommandStatus.FAILURE, False, ShapeRequest.NONE.name, ColorRequest.NONE.name

        if response[0] == CommandStatus.SUCCESS:
            if not sub_called:
                self.store_pose_shape_color_in_registers(response[2], response[3], response[4])
                self.__set_command_done(response)
                return
            else:
                return response[0], response[2], response[3], response[4]

        elif response[0] == CommandStatus.OBJECT_NOT_FOUND:
            if not sub_called:
                self.store_pose_shape_color_in_registers(response[2],
                                                         ShapeRequest.NONE.name,
                                                         ColorRequest.NONE.name)
            else:
                return response[0], response[2], ShapeRequest.NONE.name, ColorRequest.NONE.name

        elif response[0] == CommandStatus.MARKERS_NOT_FOUND:
            rospy.logwarn_throttle(1, 'ROS Wrapper - Markers Not Found')
        elif response[0] == CommandStatus.VIDEO_STREAM_NOT_RUNNING:
    ...
    def get_ws_ratio(self, values):
        ...
        if response[0] == CommandStatus.SUCCESS:
            ...
            rospy.logwarn('Modbus Server - Unable to get workspace ratio : %s', response[1])
            return response
